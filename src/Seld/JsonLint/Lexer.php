...
class Lexer
{
        ...
        while (true) {
            ...
            switch ($symbol) {
                ...
                case self::T_OPEN_COMMENT:
                    if (!($this->flags & Json_Parser::ALLOW_COMMENTS)) {
                        $this->parse_error('Lexical error on line ' . ($this->yylineno + 1) . ". Comments are not allowed.\n" . $this->show_position());
                    }
                    $this->skip_until($symbol === self::T_COMMENT ? self::T_BREAK_LINE : self::T_CLOSE_COMMENT);
                    if ($this->done) {
                ...
                case self::T_CLOSE_COMMENT:
                    $this->parse_error('Lexical error on line ' . ($this->yylineno + 1) . ". Unexpected token.\n" . $this->show_position());
                // no break
                default:
                    return $symbol;
            }
    ...
    public function show_position()
    {
        ...
        return $pre . str_replace("\n", '', $this->get_upcoming_input()) . "\n" . $c . '^';
    }
    /**
     * @return string
     */
    public function get_past_input()
    {
        $past_length = $this->offset - \strlen($this->match);
        return ($past_length > 20 ? '...' : '') . substr($this->input, max(0, $past_length - 20), min(20, $past_length));
    }
    /**
     * @return string
     */
    public function get_upcoming_input()
    {
        $next = $this->match;
        if (\strlen($next) < 20) {
     ...
     * @return string
     */
    public function get_full_upcoming_input()
    {
        $next = $this->match;
        if (substr($next, 0, 1) === '"' && substr_count($next, '"') === 1) {
            ...
            if ($len === $this->offset) {
                ...
                $str_end = min(strpos($this->input, '"', $this->offset + 1) ?: $len, strpos($this->input, "\n", $this->offset + 1) ?: $len);
            }
            $next .= substr($this->input, $this->offset, $str_end - $this->offset);
     ...
     * @return never
     */
    protected function parse_error($str)
    {
        throw new Parsing_Exception($str);
    }
    /**
     * @param int $token
     * @return void
     */
    private function skip_until($token)
    {
        $symbol = $this->next();
    ...
    private function next()
    {
        ...
        for ($i = 0; $i < $rules_len; $i++) {
            if (preg_match($this->rules[$i], $this->input, $match, 0, $this->offset)) {
                ...
                $this->yylloc = ['first_line' => $this->yylloc['last_line'], 'last_line' => $this->yylineno + 1, 'first_column' => $this->yylloc['last_column'], 'last_column' => $line_count > 0 ? \strlen($lines[$line_count - 1]) : $this->yylloc['last_column'] + \strlen($match[0])];
                $this->yytext .= $match[0];
                $this->match .= $match[0];
                $this->yyleng = \strlen($this->yytext);
                $this->more = false;
                $this->offset += \strlen($match[0]);
                return $this->perform_action($i);
            }
        }
        if ($this->offset === \strlen($this->input)) {
            return self::EOF;
        }
        $this->parse_error('Lexical error on line ' . ($this->yylineno + 1) . ". Unrecognized text.\n" . $this->show_position());
    }
    /**
     * @param  int $rule
     * @return 0|3|4|6|8|10|11|14|17|18|21|22|23|24|30|31|32|-1
     */
    private function perform_action($rule)
    {
        switch ($rule) {
            case 0:
                /* skip break line */
